...
I used this repo. Thank you
*/

static const unsigned int OPCODES_CYCLES[256] = {
...
unsigned int 8080_rb(CPU * const c, unsigned int addr) {
  return c-> bus_load(c-> userdata, addr, sizeof(unsigned char));
}

// writes a byte to memory
void 8080_wb(CPU* const cpu, uint16_t addr, uint8_t val) {
  c->write_byte(c->userdata, addr, val);
}

// reads a word from memory
unsigned int i8080_rw(i8080* const c, uint16_t addr) {
